...
class NetworksA(nn.Module):
    def __init__(self, 
        ...
        self.mode = mode
        self.type = type
        self.in_features_dict = {IN_FEATURES[i]: in_features_len[i] for i in range(len(in_features_len))}
        self.in_features = sum(in_features_len)
    ...
    def forward(self, coords, params=None):
        ...
        if self.mode == 'fft':
            ...
            if 'scale' in self.in_features_dict and 'rotation' in self.in_features_dict:
                ...
                """
                self.scale_upperbound = coords[:, xyz_len+opac_len+rgb_len:xyz_len+opac_len+rgb_len+scale_len]
