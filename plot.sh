...
model_path_base=/data1/zhangao/outputs/develop3_multi_angle
model_path_ext=develop3
save_dir=develop3_multi_angle

find_model_path_by_selection() {
    local scene_name="$1"
    local test_selection="$2"
    local scene_model_dir="${model_path_base}/${scene_name}"
    ...
    find "$scene_model_dir" -mindepth 1 -maxdepth 1 -type d -name "*${test_selection}*" | sort | head -n 1
}
...
do
    ...
    do
        ...
        model_path=$(find_model_path_by_selection "$scene_name" "$test_selection")
        if [ -z "$model_path" ]; then
            echo "No model path found for scene=${scene_name}, test_selection=${test_selection} under ${model_path_base}/${scene_name}. Skipping."
            continue
        fi
        python plot_gaussian_pts.py \
            --model_path $model_path \
            --source_path $source_path \
            --test_selection_path $test_selection_path \
            --output /data1/zhangao/outputs/plots_multiangle/${save_dir}/${scene_name}/${test_selection}.png
    done
done
...
# cap_max4=1000000
# for test_selection in test_selection_10 test_selection_20 test_selection_30 test_selection_40 test_selection_50 test_selection_60 test_selection_70 test_selection_80 test_selection_90 test_selection_100
# do
...
# model_path_base=/data1/zhangao/outputs/develop3_discontinuous_capmax
# model_path_ext=develop3
...
# # cap_max9=3300000
# cap_max9=700000
# for source_path in $source_path8 $source_path9
# do
#     if [ $source_path == $source_path1 ]; then
#         cap_max=$cap_max1
#     elif [ $source_path == $source_path2 ]; then
