...
def main():
    ...
    parser.add_argument("--ply_path", type=str, default=None, help="Path to point cloud PLY for visualization")

    args = parser.parse_args()

    # Flatten image names in case they were passed as a single quoted string
    flattened = []
    for item in args.image_name:
        flattened.extend(item.split())
    args.image_name = flattened
    ...
    for angle_deg in args.angle:
        ...
        must_keep_names = selected_names | target_names_set
        ...
        angle_tag = str(angle_deg * 2)
        fig_path = os.path.join(plots_dir, f"discontinuous_angle_{angle_tag}.png")
        fig.savefig(fig_path, dpi=1000, bbox_inches="tight", pad_inches=0.0, transparent=True)
        print(f"Saved plot to {fig_path}")

        topdown_path = os.path.join(plots_dir, f"discontinuous_angle_{angle_tag}_topdown.png")
